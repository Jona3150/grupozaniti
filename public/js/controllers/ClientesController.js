...
zanitiApp.controller('ClientesController', ['$scope', '$http', function($scope, $http) {
    ...
    $scope.guardarCliente = function() {
        ...
        // Clonamos el objeto para procesar el valor de "Otro" sin afectar el formulario visualmente
        let datosAEnviar = angular.copy($scope.nuevoCliente);

        // Si el usuario seleccionó 'Otro', tomamos el valor escrito en el campo de texto adicional
        ...
        $http.post(url, datosAEnviar).then(function(response) {
            ...
            // Notificación opcional (puedes usar un toast si lo prefieres)
            console.log($scope.editando ? "Cliente actualizado" : "Cliente registrado");
        }, function(error) {
